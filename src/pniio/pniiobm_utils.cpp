...
//----------------------------------------------------------------------------
void write_parameters(BenchmarkLog &log,const configuration &config)
{
...
//----------------------------------------------------------------------------
benchmark_runner create_runner(const BenchmarkPointer &ptr)
{
  benchmark_runner runner;

  //create the pre- and post-run functions
  BenchmarkFunction pre_run = std::bind(&FileIOBenchmark::create,ptr.get());
  BenchmarkFunction post_run = std::bind(&FileIOBenchmark::close,ptr.get());
                   ...
                   const string &cli_args)
{
  ...
  BenchmarkLog log = f.create_log(config.value<string>("entry"),
